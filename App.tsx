...
import { Search, Plus, Trash2, Edit3, MessageSquare, History, Settings, ExternalLink, Download, FileJson, FileText, FileCode, ChevronDown } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const exportChat = (format: 'txt' | 'json' | 'md') => {
    ...
    let content = "";
    let fileName = `${activeChat.name.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}`;
    ...
    if (format === 'json') {
      ...
      content = `# ${activeChat.name}\n\n`;
      activeChat.messages.forEach(msg => {
        content += `## ${msg.role === 'user' ? 'User' : activeChat.persona}\n`;
        msg.parts.forEach(part => {
          if (part.text) content += `${part.text}\n\n`;
          if (part.image) content += `(Image manifestation generated by AI)\n\n`;
        });
        content += `---\n\n`;
      });
      fileName += ".md";
    } else {
      content = `${activeChat.name}\n${'='.repeat(activeChat.name.length)}\n\n`;
      activeChat.messages.forEach(msg => {
            ...
            <div className="relative">
              <button
                ...
                className={cn(
                  ...
                  isExportMenuOpen ? "bg-white/10 border-white/20 text-white" : "bg-white/5 border-white/10 text-slate-400 hover:text-white"
                )}
              ...
              {isExportMenuOpen && (
                <div className="absolute right-0 mt-2 w-48 glass-card rounded-2xl border border-white/10 shadow-2xl py-2 z-50 animate-in fade-in zoom-in-95">
                  <div className="px-4 py-2 border-b border-white/5 mb-1">
                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Choose Format</p>
                  </div>
                  <button 
                    onClick={() => exportChat('txt')}
                    className="w-full flex items-center gap-3 px-4 py-2 text-xs text-slate-300 hover:bg-white/5 hover:text-cyan-400 transition-colors"
                  >
                  ...
                  <button 
                    ...
                    className="w-full flex items-center gap-3 px-4 py-2 text-xs text-slate-300 hover:bg-white/5 hover:text-violet-400 transition-colors"
                  >
                  ...
                  <button 
                    ...
                    className="w-full flex items-center gap-3 px-4 py-2 text-xs text-slate-300 hover:bg-white/5 hover:text-fuchsia-400 transition-colors"
                  >
                    <FileJson className="w-4 h-4" />
                    <span>Data Object (.json)</span>
                  </button>
                </div>
              )}
            </div>
